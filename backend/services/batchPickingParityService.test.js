...
(async () => {
  ...
  const success = await deductBatchPickingAndMIS({
    ...
    payload: { warehouse: 'BB04', salesDocument: 'SO-1', obdNumber: '12345678', updatedBy: 'TEST',
  ...
  assert.equal(success.status, 'SUCCESS'); assert.equal(success.transaction, 'COMMITTED');
  assert.equal(ok.state.stock[0].batch_qty[0].qty, 20); assert.equal(ok.state.stock[0].total_unrestricted, 20);
  assert.equal(ok.state.operation.order_status, 'PGI Done'); assert.equal(ok.state.mis.length, 1);
  assert.equal(ok.state.committed, true); assert.equal(ok.state.rolledBack, false); assert.equal(ok.state.released, true);

  const bad = makeDb({ failMis: true });
  const failure = await deductBatchPickingAndMIS({
    db: bad.db, mapping: bad.mapping,
    payload: { warehouse: 'BB04', salesDocument: 'SO-1', obdNumber: '87654321',
      allocatedRows: [{ sku: 'SKU1', batch: '123', allocQty: 10, sloc: 'FG01' }], opRowData: { plant: 'BB04', orderQty: 100 } }
  });
  assert.equal(failure.status, 'ERROR'); assert.equal(failure.transaction, 'ROLLED_BACK');
  assert.equal(bad.state.stock[0].batch_qty[0].qty, 70); assert.equal(bad.state.stock[0].total_unrestricted, 70);
  assert.equal(bad.state.rolledBack, true); assert.equal(bad.state.committed, false); assert.equal(bad.state.mis.length, 0); assert.equal(bad.state.released, true);
